...
import { DreamCreate, Dream } from '../services/api'
import { format, addDays } from 'date-fns'
import ProgressBar from './ProgressBar'
// @ts-ignore - SavingsBreakdown.jsx doesn't have TypeScript declarations
...
export default function DreamBuilder({ onComplete, selectedTemplate }: DreamBuilderProps) {
  const [step, setStep] = useState(1)
  const [formData, setFormData] = useState<DreamCreate>({
    title: '',
    description: '',
    category: 'lifestyle',
    target_amount: 0,
  ...
  const [savingsCalculations, setSavingsCalculations] = useState<any>(null)
  const [selectedStrategy, setSelectedStrategy] = useState<'aggressive' | 'balanced' | 'relaxed'>('balanced')
  const [lifeEquivalents, setLifeEquivalents] = useState<any[]>([])
  ...
  useEffect(() => {
    if (formData.target_amount > 0 && formData.target_date) {
      try {
        ...
        // Calculate life equivalents for the balanced strategy
        ...
        // Reset calculations if there's an error (e.g., invalid date)
  ...
  // Load existing draft on component mount or populate with template
  useEffect(() => {
    const loadSavedDraft = async () => {
      try {
        // If a template is selected, use it instead of loading draft
        if (selectedTemplate) {
          // Map template category to our category values
          const categoryMapping: { [key: string]: 'travel' | 'home' | 'education' | 'family' | 'freedom' | 'lifestyle' | 'health' } = {
            'Travel': 'travel',
          ...
          // Calculate target date from suggested timeframe
          const targetDate = format(addDays(new Date(), selectedTemplate.suggestedTimeframe * 30), 'yyyy-MM-dd')
          
          setFormData({
            title: selectedTemplate.title,
            description: selectedTemplate.description,
            category: categoryMapping[selectedTemplate.category] || 'lifestyle',
            target_amount: selectedTemplate.amount,
            target_date: targetDate,
            image_url: selectedTemplate.imageUrl || ''
          })
          
          setDraftLoaded(true)
          console.log('Template loaded successfully:', selectedTemplate.title)
          return
        }
        ...
        if (draft) {
          ...
          // Restore selected strategy if available
          if (draft.selectedStrategy) {
            setSelectedStrategy(draft.selectedStrategy)
          }
          
          // Restore step if available (but don't go past step 4)
    ...
    loadSavedDraft()
  }, [selectedTemplate]) // Re-run when selectedTemplate changes

  // Auto-save draft with debouncing (500ms)
  useEffect(() => {
    // Don't auto-save immediately after loading draft or if no meaningful data
    if (!draftLoaded && (!formData.title && !formData.description && formData.target_amount === 0)) {
      return
    }

    // Don't save if dream is already created
    if (createdDream) {
      return
    }
      ...
      const saveDraft = async () => {
        ...
        try {
          const draftData = {
            ...formData,
            selectedStrategy,
            step // Include current step for better UX
          }
                ...
                <div className="space-y-8">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                      {Object.entries(savingsCalculations).filter(([key]) => key !== 'metadata').map(([key, strategy]: [string, any]) => (
                        <div
                          key={key}
                        ...
                        >
                          <div className="flex items-center mb-3">
                            <input
                              ...
                              onChange={() => setSelectedStrategy(key as any)}
                              className="mr-3"
                            />
                            <div>
                    ...
                    <div>
                      ...
                      <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-6 rounded-lg">
                        ...
                        <div className="space-y-4">
                          {milestones.milestones.slice(0, 3).map((milestone: any, index: number) => (
