...
const FinancialRealityWizard = ({ onComplete, onBack }) => {
  ...
  useEffect(() => {
    if (financialProfile && debts.length > 0) {
      ...
      updatedProfile.updateCalculatedFields();
      
      localStorage.setItem('financialProfile', JSON.stringify(updatedProfile.toJSON()));
  ...
  const calculateFreedomRatio = (currentExpenses = expenses) => {
    ...
    const monthlyIncome = financialProfile.userProfile.getMonthlyNetIncome();
  ...
  const addDebt = () => {
    ...
    const newDebt = new Debt({
      ...
      currentBalance: parseFloat(currentDebt.currentBalance),
      minimumPayment: parseFloat(currentDebt.minimumPayment),
      monthlyPayment: parseFloat(currentDebt.minimumPayment), // Start with minimum
      interestRate: parseFloat(currentDebt.interestRate) || 0
    });
  ...
  const calculatePayoffTimelines = () => {
    ...
    const avalancheStrategy = updatedProfile.calculateDebtPayoffTimeline('avalanche', 0);
    const snowballStrategy = updatedProfile.calculateDebtPayoffTimeline('snowball', 0);
                ...
                onClick={() => {
                  ...
                  if (expenses.everythingElse && !expenses.groceries) {
                    ...
                    const estimatedBreakdown = {
                      groceries: Math.round(everythingElseAmount * 0.3),
                      utilities: Math.round(everythingElseAmount * 0.2),
                      transportation: Math.round(everythingElseAmount * 0.25),
                      insurance: Math.round(everythingElseAmount * 0.15),
                      subscriptions: Math.round(everythingElseAmount * 0.05),
                      other: Math.round(everythingElseAmount * 0.05)
                    };
              ...
              <div className="text-center">
                <button
                  onClick={() => {
                    ...
                    setExpenses(prev => ({ 
                      ...prev, 
                      everythingElse: detailedTotal.toString(),
